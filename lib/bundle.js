...
class Ship extends MovingObject {
  constructor(stage, options) {
    ...
    options.shape.graphics.beginFill('white').arc(0, 0, 15, 0, 6.29).endFill();
...
class Game {
  ...
  randomEnemyPosition(level) {
    ...
    if (level === 0) {
      ...
      posX = Util.randomInRange(this.DIM_X - 100, this.DIM_X);
      posY = Util.randomInRange(1, 100);
    } else if (level === 3) {
      posX = Util.randomInRange(this.DIM_X - 100, this.DIM_X);
      posY = Util.randomInRange(this.DIM_Y - 100, this.DIM_Y);
    } else {
      posX = Util.randomInRange(1, 100);
      posY = Util.randomInRange(this.DIM_Y - 100, this.DIM_Y);
    }
  ...
  explode(bomb) {
    ...
    this.createExplosion(bomb.pos[0], bomb.pos[1], "#FF00FF");
    this.createExplosion(bomb.pos[0], bomb.pos[1], "#9370DB	");
    this.remove(bomb);
  }
...
class Bomb extends MovingObject {
  constructor(stage, options) {
    options.shape = new createjs.Shape().set(options.pos);
    options.shape.graphics.beginFill('red').drawPolyStar(0, 0, 18, 8, .3, -15);
